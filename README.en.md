...
This repository is the **release front**: docs and the Windows installers
([Releases](https://github.com/tnqaquocanh-vn/theisle-overlay/releases)) only.
Development happens in BumBum's private repository.

Licensed **MIT** (see [`LICENSE`](LICENSE)) — you may fork, modify and build
from the previously published source snapshots; keep the copyright notice and
the attributions in [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md). Bugs /
suggestions via [GitHub Issues](https://github.com/tnqaquocanh-vn/theisle-overlay/issues).
...
<!-- TODO(BumBum): add your own support method (VietQR / Ko-fi / …) and QR
     image under docs/. The previous donate image has been removed. -->
Some advanced features are supporter-only (see the *Supporters* section).
