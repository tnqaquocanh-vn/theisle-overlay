...
pub struct AppState {
    ...
    /// (map heading deg, sample time in `now_s`) from the last G1 local-capture
    /// sample. The exact control yaw beats movement heading while fresh.
    pub last_exact_heading: Mutex<Option<(f64, f64)>>,
    /// `now_s` of the last G1 local-capture position sample. While this is
    /// fresh, remote providers keep feeding vitals but stop overwriting
    /// position — local capture is more accurate and far more frequent (P2).
    pub last_localpos_sample: Mutex<Option<f64>>,
    /// `now_s` of the last IslePilot realtime-socket position sample (G5).
    /// While fresh, the slower REST poll skips its own position ingest so the
    /// marker doesn't hop backward once every poll interval.
    pub last_realtime_sample: Mutex<Option<f64>>,
    started: Instant,
...
impl AppState {
    pub fn new() -> Self {
        ...
        Self {
            ...
            last_exact_heading: Mutex::new(None),
            last_localpos_sample: Mutex::new(None),
            last_realtime_sample: Mutex::new(None),
