...
/// Run a hotkey action by name from outside this module (G7 mouse gestures
/// reuse `zoom_in` / `zoom_out` / `toggle_minimap`).
pub fn trigger(app: &AppHandle, action: &str) {
    dispatch(app, action);
}
...
fn dispatch(app: &AppHandle, action: &str) {
    ...
    match action {
        ...
        "team_ping" => crate::team::ping_here(app),
        "cycle_preset" => crate::commands::cycle_preset(app),
        "toggle_bigmap" => crate::bigmap::toggle(app),
        "toggle_companion" => crate::companion::toggle(app),
        // B5: ask the minimap webview to read its canvas and ship the pixels
        // to copy_map_snapshot — the webview has the frame, Rust has the
        // clipboard (the overlay window has no focus for navigator.clipboard).
        "map_snapshot" => {
            let _ = app.emit("minimap://snapshot", ());
        }
