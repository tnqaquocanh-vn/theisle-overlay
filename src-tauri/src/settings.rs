...
pub fn default_settings() -> Value {
    json!({
        "minimap": {
            ...
            "basemap_px": 2600,         // decode resolution of the disc's basemap (975 - 3900);
                                        // vulnona tier-3 is 3900 px, tier-1 (old default) was 975

            "click_through": true,
            "show_trail": true,          // trail lines on the minimap disc
            "show_waypoints": true,      // waypoint dots + nearest-waypoint arrow
            "rotate_with_heading": false, // rotate the disc so travel points up
            "show_team_panel": true,     // G6: teammate HP/hunger/thirst strip under the disc
            "mouse_gestures": false,     // G7: Alt+wheel zoom / Alt+MMB show-hide (Raw Input, opt-in)
            "last_seen_beacon": true,    // P6: drop a waypoint at the last position when the signal drops
            "smooth_motion": false,      // P1: ease the marker between samples instead of teleporting
            "diagnostics": false,        // v1.26: render-ms / repaint-rate readout on the disc
            "solo_mode": false,          // A5: hide teammates / party dots / pings from the HUD
            "auto_preset": false,        // A4: apply the preset named after your species on a swap
            "panel_order": ["dino", "quests", "team"], // A2: stack order of the HUD panels
        },
        // G1: automatic own-position from passive UDP capture. OFF by default
        // and gated behind a disclaimer in Settings — it needs Npcap and reads
        // network packets (never the game process).
        "localpos": {
            "enabled": false,
        },
        // G6: ad-hoc team relay. `relay_base` is the user's own deployed
        // worker (see worker/README) — empty until they set it, feature stays
        // hidden until then.
        "team": {
            "relay_base": "",
        },
        // v1.25: the in-game big map (Ctrl+Alt+G). Only the dimmed panel's
        // opacity is user-tunable for now (0.6 - 1.0).
        "bigmap": {
            "opacity": 0.96,
        },
        ...
        "hotkeys": {
            ...
            "team_ping": "Ctrl+Alt+X",   // G6: drop a contact ping for the team
            "cycle_preset": "Ctrl+Alt+P", // P5/A4: apply the next saved overlay preset
            "map_snapshot": "Ctrl+Alt+S", // B5: copy the minimap frame to the clipboard
            "toggle_bigmap": "Ctrl+Alt+G", // v1.25: the in-game big map
            "toggle_companion": "Ctrl+Alt+D", // A7: the 2nd-monitor companion dashboard
        ...
        "layers": {
            ...
            "explored": false,           // fog-of-war grid of visited cells
        },
        "map": {
            "zone_labels": true,         // names inside zone outlines
            "basemap": "vulnona",        // vulnona | islemaps_light | islemaps_dark
            "show_cursor_coords": false, // live game-coord readout under the cursor
        },
        "trail": {
            "enabled": true,
        ...
        // A10: opt-in HUD sound cues (teammate ping / low HP / signal lost).
        "sound": {
            "enabled": false,
        },
        // In-app auto-update: check the signed release feed once on startup.
        // The manual "Check for updates" button in Settings always works.
        "updates": {
            "auto_check": true,
        },
        "number_format": "auto",         // auto | us | eu
        "language": "vi",                // vi | en
        "color_profile": "default",      // default | deuteranopia (map layer palette)
        "skin": "obsidian",              // A9: obsidian | bonefield | biolum (ground palette)
        // A1: the first-run wizard. false only on a truly fresh install —
        // load_settings() forces it true when an existing settings.json has no
        // such key, so an upgrade never pops the wizard. The "Run setup again"
        // button in Settings writes it back to false.
        "onboarding_done": false,
        "hidden_waypoint_groups": [],    // waypoint folders hidden on the maps
        ...
        "islepilot": {
            ...
            "poll_interval_s": 10,
            "realtime": true,            // token mode: also use the wss://islepilot.eu/ows socket (G5)
            ...
            "show_party": false,         // other players' live-map positions (opt-in)
            "history_enabled": true,     // log stat samples for the Your Dino charts
            "history_days": 14,          // retention window, pruned at startup
            "death_marker": true,        // A5: drop + share a 💀 waypoint when the dino dies
            // Threshold notifications (opt-in). *_pct = 0 turns that rule off.
            "alerts": {
                "enabled": false,
                "thirst_pct": 15,
                "hunger_pct": 15,
...
pub fn load_settings() -> Value {
    ...
    match loaded {
        Some(over @ Value::Object(_)) => {
            ...
            // An existing settings.json means the user is already set up —
            // never pop the first-run wizard at them on upgrade. Only a fresh
            // install (no file at all, the arm below) starts at false.
            if over.get("onboarding_done").is_none() {
                merged["onboarding_done"] = Value::Bool(true);
            }
            merged
        }
        _ => default_settings(),
...
mod tests {
    ...
    #[test]
    fn onboarding_done_defaults_false_but_merge_never_re_flags_an_upgrade() {
        // Fresh install: the wizard should run.
        assert_eq!(default_settings()["onboarding_done"], false);
        // A legacy file with no such key: merge alone leaves it false — it is
        // load_settings() that forces it true so an upgrade is not nagged.
        let legacy = json!({ "language": "en" });
        assert_eq!(merge(&default_settings(), &legacy)["onboarding_done"], false);
        // A file that already recorded a choice keeps it.
        let seen = json!({ "onboarding_done": true });
        assert_eq!(merge(&default_settings(), &seen)["onboarding_done"], true);
    }
}
