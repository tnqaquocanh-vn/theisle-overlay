...
pub fn create(app: &AppHandle) -> tauri::Result<()> {
    ...
    build_window(app, snap.window_w(), snap.window_h())?;
...
/// Height of the dino-stats strip under the map disc, logical px. The webview
/// (`drawDinoPanel` in src/minimap/render.ts) lays its rows out from this;
/// there is no matching constant on that side any more. Covers the identity
/// line, the status/Prime chip row, three vital rows and the growth gauge.
const DINO_PANEL_H: f64 = 105.0;
/// One extra stats row (stamina, token mode only).
const DINO_PANEL_ROW_H: f64 = 16.0;
/// One extra line for the "eat next" nutrition hint (token mode only).
const DINO_NUTRI_ROW_H: f64 = 18.0;
...
/// The webview shows unfinished quests first and no more than this many (a
/// 19-quest list is a wall of tiny text). Mirror: QUEST_MAX_ROWS in
/// src/minimap/render.ts.
const QUEST_MAX_ROWS: usize = 10;

/// Height of the Prime-quests panel for `n` quests; 0 quests -> no panel at
/// all (an empty card under the minimap would just be clutter). Row count is
/// capped at QUEST_MAX_ROWS so the card cannot run off-screen.
fn quests_panel_h(n: usize) -> f64 {
    if n == 0 {
        0.0
    } else {
        QUEST_HEADER_H + n.min(QUEST_MAX_ROWS) as f64 * QUEST_ROW_H + QUEST_PAD_H
    }
}

/// Teammate-stats panel geometry, logical px. Must match TEAM_HEADER_H /
/// TEAM_ROW_H / TEAM_PAD_H in src/minimap/render.ts.
const TEAM_HEADER_H: f64 = 16.0;
const TEAM_ROW_H: f64 = 20.0;
const TEAM_PAD_H: f64 = 6.0;

fn team_panel_h(n: usize) -> f64 {
    if n == 0 {
        0.0
    } else {
        TEAM_HEADER_H + n as f64 * TEAM_ROW_H + TEAM_PAD_H
    }
}

/// The three stacked-panel heights the minimap webview needs to size its
/// canvas and place each card. This crate is the SINGLE source of truth for
/// them: `render.ts` used to recompute the same three totals from its own copy
/// of the constants, and a one-sided edit clipped a panel or left a black gap.
/// Now the webview fetches this at startup (`minimap_layout`) and follows the
/// `minimap://layout` push on every change; it keeps only the within-card row
/// pitch locally.
#[derive(serde::Serialize, Clone, Copy, PartialEq)]
#[serde(rename_all = "camelCase")]
pub struct MinimapLayout {
    /// "Your dino" stats strip, logical px (0 = panel off).
    pub panel_h: f64,
    /// Prime-quests card, logical px (0 = off or no quests).
    pub quests_h: f64,
    /// Teammate-stats strip, logical px (0 = off or solo).
    pub team_h: f64,
}

fn layout_from_settings(s: &serde_json::Value) -> MinimapLayout {
    let ip_on = settings::get_bool(s, &["islepilot", "enabled"], false);
    MinimapLayout {
        panel_h: if ip_on && settings::get_bool(s, &["islepilot", "show_overlay_panel"], true) {
            DINO_PANEL_H
                ...
                + if crate::islepilot::last_has_nutrition() {
                    DINO_NUTRI_ROW_H
                } else {
                    0.0
                }
        ...
        quests_h: if ip_on && settings::get_bool(s, &["islepilot", "show_quests_panel"], false) {
            quests_panel_h(crate::islepilot::last_quest_count())
        } else {
            0.0
        },
        team_h: if settings::get_bool(s, &["minimap", "show_team_panel"], true)
            && !settings::get_bool(s, &["minimap", "solo_mode"], false)
        {
            team_panel_h(crate::team::overlay_rows())
        } else {
            0.0
        },
    }
...
/// Initial panel geometry for a freshly (re)loaded minimap webview — it then
/// follows `minimap://layout` for every later change.
#[tauri::command]
pub fn minimap_layout(app: AppHandle) -> MinimapLayout {
    let state = app.state::<AppState>();
    let s = state.settings.lock_safe();
    layout_from_settings(&s)
}
...
struct Snapshot {
    ...
    quests_h: f64,
    /// Extra height for the teammate-stats panel (G6, varies with team size).
    team_h: f64,
    /// G8: uniform HUD scale (0.65–1.75). Multiplies the whole overlay.
    hud_scale: f64,
}

impl Snapshot {
    /// Logical width of the overlay window (before DPI).
    fn window_w(&self) -> f64 {
        self.size_px * self.hud_scale
    }
    /// Logical height of the overlay window (before DPI).
    fn window_h(&self) -> f64 {
        (self.size_px + self.panel_h + self.quests_h + self.team_h) * self.hud_scale
    }
    fn layout(&self) -> MinimapLayout {
        MinimapLayout {
            panel_h: self.panel_h,
            quests_h: self.quests_h,
            team_h: self.team_h,
        }
    }
}
...
fn snapshot(app: &AppHandle) -> Snapshot {
    let state = app.state::<AppState>();
    let s = state.settings.lock_safe();
    // The 250 ms tick picks up stamina / quest-count / team-size changes via
    // the diff against the previous snapshot.
    let layout = layout_from_settings(&s);
    Snapshot {
        ...
        quests_h: layout.quests_h,
        team_h: layout.team_h,
        hud_scale: settings::get_f64(&s, &["minimap", "hud_scale"], 1.0).clamp(0.65, 1.75),
    }
}
        ...
        loop {
            ...
            let Some(window) = app.get_webview_window("minimap") else {
                ...
                if since_recreate >= RECREATE_MS {
                    ...
                    match build_window(&app, cur.window_w(), cur.window_h()) {
                        Ok(w) => {
            ...
            if !size_applied
                || cur.window_w() != prev.window_w()
                || cur.window_h() != prev.window_h()
            {
                size_applied = true;
                let _ = window.set_size(LogicalSize::new(cur.window_w(), cur.window_h()));
                last_rect = None;
            }
            // The webview follows this for canvas size + card placement (it
            // also fetches minimap_layout once at startup). Push on any change.
            if !layout_sent || cur.layout() != prev.layout() {
                layout_sent = true;
                let _ = window.emit("minimap://layout", cur.layout());
            }
...
fn anchor(window: &tauri::WebviewWindow, rect: (i32, i32, i32, i32), snap: &Snapshot) {
    ...
    let size = (snap.window_w() * scale).round() as i32;
...
mod tests {
    use super::{
        quests_panel_h, GamePresence, QUEST_HEADER_H, QUEST_MAX_ROWS, QUEST_PAD_H, QUEST_ROW_H,
    };
    ...
    fn quest_panel_height_scales_with_count_and_vanishes_at_zero() {
        ...
        // Long quest lists are capped so the card can't run off-screen.
        assert_eq!(quests_panel_h(19), quests_panel_h(QUEST_MAX_ROWS));
        assert_eq!(quests_panel_h(QUEST_MAX_ROWS + 5), quests_panel_h(QUEST_MAX_ROWS));
    }
