...
## Nhóm sinh tồn (G6) — Durable Object relay

`src/team.ts` thêm một **Durable Object** `TeamRoom`: mỗi mã mời 6 ký tự là một
phòng, chỉ nằm trong RAM (state ở attachment của WebSocket, không đụng storage),
phòng trống thì tự evict.

Route:

- `POST /v1/team/new` → `{ "code": "ABC123" }` (sinh mã ngẫu nhiên, chưa tạo DO).
- `GET  /v1/team/ws?code=ABC123` (nâng cấp WebSocket) → vào phòng theo mã.

Frame JSON: `hello` / `tele` / `ping` vào; `welcome` / `roster` ra (xem đầu file
`team.ts`). Tối đa 10 người/phòng, hết heartbeat 30 giây thì bị đá.

### Deploy

DO SQLite-backed **có ở Workers Free**. `wrangler.jsonc` đã khai báo binding
`TEAM` + migration `v1`. Chỉ cần:

```bash
npm run deploy      # wrangler deploy — lần đầu sẽ chạy migration tạo class
```

Sau đó lấy URL worker (`https://<name>.<account>.workers.dev`) dán vào ứng dụng:
*Khủng long → Nhóm sinh tồn → Địa chỉ relay*. Không cần secret gì thêm; relay
không ghi D1/AE nên không tốn quota telemetry.

### Cách nhanh nhất: chỉ deploy relay nhóm

Nếu tài khoản Cloudflare của bạn chưa bật Analytics Engine / chưa có D1 (worker
đầy đủ cần các thứ đó cho telemetry), dùng bản relay tối giản — chỉ có Durable
Object, không cần bật gì thêm:

```bash
npm run deploy:team
```

→ `https://isle-team-relay.<subdomain>.workers.dev`. Dán URL này vào app:
*Khủng long → Nhóm sinh tồn → Nâng cao → Địa chỉ relay*.

Kiểm tra: `curl -X POST https://isle-team-relay.<subdomain>.workers.dev/v1/team/new`
→ ra `{"code":"..."}` là xong.
